...
export function BookingSection() {
  ...
  const syncRangeFromManualInput = (type: "from" | "to", rawValue: string) => {
    ...
    setRange((current) => {
      const safeFrom = current?.from && nextDate < current.from ? nextDate : current?.from ?? nextDate;
  ...
  const handleChildrenChange = (value: number) => {
    ...
    setChildrenAges((previousAges) => previousAges.slice(0, safeChildren).concat(Array(Math.max(0, safeChildren - previousAges.length)).fill(0)));
  };
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const normalizedChildrenAges = childrenAges.slice(0, children).map((age) => Math.max(0, Number(age) || 0));
    trackBookingStart({
      path: window.location.pathname,
  ...
  return (
    <section id="foglalas" className="relative border-t border-border/50 py-28 md:py-40">
      <div className="mx-auto max-w-7xl px-6">
        <SectionHeading
          eyebrow="Foglalás"
          title="Válaszd ki a dátumot"
          lead="A szürke napok már foglaltak. A kérelmedet 24 órán belül visszaigazoljuk."
        />
            ...
            <div className="mt-4 border-t border-border/60 pt-4 text-sm">
              {nights > 0 ? (
                <div className="flex items-center justify-between">
                  ...
                  <span className="ml-4 shrink-0 font-display text-2xl text-primary">{formatHuf(pricingSummary.total)}</span>
                </div>
              ) : (
          ...
          <Reveal delay={140}>
            <form onSubmit={handleSubmit} className="grid gap-5 sm:grid-cols-2">
              <div className="sm:col-span-1">
                ...
                <Input id="name" name="name" required className="mt-2" placeholder="Kovács Anna" />
              </div>
              <div className="sm:col-span-1">
              ...
              <div>
                ...
                <select
                  ...
                  className="mt-2 flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                >
                  {Array.from({ length: MAX_TOTAL_GUESTS + 1 }, (_, index) => index).map((value) => (
                    <option key={value} value={value}>
                      {value}
                    </option>
                  ))}
              ...
              <div className="sm:col-span-1">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Max. kapacitás: {MAX_TOTAL_GUESTS} fő · Min. 2 éjszaka</p>
              </div>
              {children > 0 && (
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {Array.from({ length: children }, (_, index) => (
                      <div key={`child-age-${index}`}>
                        <Label htmlFor={`child-age-${index}`} className="text-sm text-muted-foreground">
                ...
                <div className="sm:col-span-2 rounded-sm border border-border/70 bg-card/40 p-4">
                  ...
                  <div className="space-y-2 text-sm text-muted-foreground">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="font-medium text-foreground">{formatHuf(pricingSummary.roomSubtotal)}</span>
                    </div>
                    {pricingSummary.singleNightSurcharge > 0 && (
                      <div className="flex items-center justify-between gap-4">
                        <span>1 éjszakás felár (+50%)</span>
                        <span className="font-medium text-foreground">{formatHuf(pricingSummary.singleNightSurcharge)}</span>
                      </div>
                    )}
                    <div className="flex items-center justify-between gap-4">
                      <span>IFA</span>
                      <span className="font-medium text-foreground">{formatHuf(pricingSummary.ifaSubtotal)}</span>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                      <span>Kutya díj</span>
                      <span className="font-medium text-foreground">{formatHuf(pricingSummary.dogSubtotal)}</span>
                    </div>
                    <div className="border-t border-border/60 pt-2">
                      ...
                      <div className="mt-2 flex items-center justify-between gap-4 text-sm text-muted-foreground">
                        <span>50% előleg</span>
                        <span className="font-medium text-primary">{formatHuf(pricingSummary.deposit)}</span>
                      </div>
                    </div>
                  </div>
                </div>
